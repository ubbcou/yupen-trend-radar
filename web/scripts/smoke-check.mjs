...
await dateLedger.getByText("2026-07-27", { exact: true }).waitFor();
await dateLedger.getByText("指数", { exact: true }).waitFor();
await dateLedger.getByText("板块", { exact: true }).waitFor();
if ((await dateLedger.getByText("2026-07-24", { exact: true }).count()) !== 2) {
...
await detail.getByText("较 2026-07-23 持平", { exact: true }).waitFor();
await detail.getByText("量比 0.67", { exact: true }).waitFor();
await detail.getByText("中证A500", { exact: true }).waitFor();
await detail.getByText(/保持前三并补齐量能和文章支持/).waitFor();
...
await detail.getByText("较 2026-07-23 持平", { exact: true }).waitFor();
await detail.getByText("-18.53%", { exact: true }).waitFor();
