...
if ((await dateLedger.getByText("2026-07-14", { exact: true }).count()) !== 2) {
  throw new Error("Index and sector data dates should both be visible");
}
await page.getByText(/超跌反弹后的局部修复/).waitFor();
...
await detail.getByText("较 2026-07-13 持平", { exact: true }).waitFor();
await detail.getByText("量比 1.18", { exact: true }).waitFor();
...
await history.getByText("2026-07-08", { exact: true }).waitFor();
await history.getByText("趋势保持", { exact: true }).first().waitFor();
await history.getByText("2026-07-14", { exact: true }).waitFor();
...
await detail.locator(".metric-strip").getByText("第 7 名", { exact: true }).waitFor();
await detail.getByText("较 2026-07-13 持平", { exact: true }).waitFor();
await detail.getByText("-4.19%", { exact: true }).waitFor();
...
await mobilePage.getByRole("button", { name: /CS创新药/ }).click();
