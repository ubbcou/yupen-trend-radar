...
await dateLedger.getByText("2026-07-31", { exact: true }).waitFor();
await dateLedger.getByText("指数", { exact: true }).waitFor();
await dateLedger.getByText("板块", { exact: true }).waitFor();
if ((await dateLedger.getByText("2026-07-30", { exact: true }).count()) !== 2) {
  throw new Error("Index and sector data dates should both be visible");
}
await page.getByRole("heading", { name: "3 个主攻方向" }).waitFor();
...
await detail.getByText("较 2026-07-29 持平", { exact: true }).waitFor();
await detail.getByText("量比 1.45", { exact: true }).waitFor();
await detail.getByText("中证A500", { exact: true }).waitFor();
await detail.getByText(/避免把BOLL上轨附近加速当新起点/).waitFor();
...
await history.getByText("2026-07-30", { exact: true }).waitFor();
...
await detail.getByText("较 2026-07-29 持平", { exact: true }).waitFor();
await detail.getByText("-22.23%", { exact: true }).waitFor();
