...
class WebSnapshotTest(unittest.TestCase):
    def test_snapshot_uses_current_project_facts(self):
        ...
        self.assertEqual("2026-07-28", snapshot["meta"]["article"]["date"])
        self.assertEqual("2026-07-27", snapshot["meta"]["fishDataDates"]["index"])
        self.assertEqual("2026-07-27", snapshot["meta"]["fishDataDates"]["sector"])
        ...
        self.assertEqual(5, innovation_drug["rank"])
        self.assertEqual("2026-07-24", innovation_drug["previousDataDate"])
        self.assertEqual(-1, innovation_drug["rankMovement"])
        self.assertEqual(0.74, innovation_drug["volumeRatio"])
    ...
    def test_direction_detail_exposes_evidence_and_lifecycle(self):
        ...
        self.assertTrue(innovation_drug["sourceImage"].endswith("sector_08622953.jpg"))
    ...
    def test_snapshot_includes_display_context_without_article_body(self):
        ...
        directions = {row["name"]: row for row in snapshot["directions"]}
        self.assertIn("连续站上20日线", directions["CS创新药"]["nextValidation"])
