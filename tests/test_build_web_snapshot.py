...
class WebSnapshotTest(unittest.TestCase):
    def test_snapshot_uses_current_project_facts(self):
        ...
        self.assertEqual("2026-07-23", snapshot["meta"]["article"]["date"])
        self.assertEqual("2026-07-22", snapshot["meta"]["fishDataDates"]["index"])
        self.assertEqual("2026-07-22", snapshot["meta"]["fishDataDates"]["sector"])

        groups = {group["name"]: group["directions"] for group in snapshot["groups"]}
        self.assertEqual([], groups["主攻"])
        self.assertEqual(
            ["中证煤炭", "中证消费", "CS创新药", "上证50"],
            groups["试探"],
        )
        self.assertEqual(
            ["国企指数"],
        ...
        self.assertEqual("2026-07-21", innovation_drug["previousDataDate"])
        self.assertEqual(-1, innovation_drug["rankMovement"])
        self.assertEqual(0.76, innovation_drug["volumeRatio"])
    ...
    def test_direction_detail_exposes_evidence_and_lifecycle(self):
        ...
        self.assertTrue(innovation_drug["sourceImage"].endswith("sector_8dc1f784.jpg"))
