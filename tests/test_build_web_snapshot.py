...
class WebSnapshotTest(unittest.TestCase):
    def test_snapshot_uses_current_project_facts(self):
        ...
        self.assertEqual("2026-07-29", snapshot["meta"]["article"]["date"])
        self.assertEqual("2026-07-28", snapshot["meta"]["fishDataDates"]["index"])
        self.assertEqual("2026-07-28", snapshot["meta"]["fishDataDates"]["sector"])
        ...
        self.assertEqual([], groups["等待"])

        directions = {row["name"]: row for row in snapshot["directions"]}
        innovation_drug = directions["CS创新药"]
        self.assertEqual(7, innovation_drug["rank"])
        self.assertEqual("2026-07-27", innovation_drug["previousDataDate"])
        self.assertEqual(-2, innovation_drug["rankMovement"])
        self.assertEqual(0.69, innovation_drug["volumeRatio"])
    ...
    def test_direction_detail_exposes_evidence_and_lifecycle(self):
        ...
        self.assertTrue(innovation_drug["sourceImage"].endswith("sector_d010ac16.jpg"))
    ...
    def test_snapshot_includes_display_context_without_article_body(self):
        ...
        self.assertIn("主攻方向恢复为国企指数和恒生科技", snapshot["marketSummary"])
        directions = {row["name"]: row for row in snapshot["directions"]}
        self.assertIn("先站回20日线", directions["CS创新药"]["nextValidation"])
