...
class WebSnapshotTest(unittest.TestCase):
    def test_snapshot_uses_current_project_facts(self):
        ...
        self.assertEqual("2026-07-20", snapshot["meta"]["article"]["date"])
        self.assertEqual("2026-07-17", snapshot["meta"]["fishDataDates"]["index"])
        self.assertEqual("2026-07-17", snapshot["meta"]["fishDataDates"]["sector"])

        groups = {group["name"]: group["directions"] for group in snapshot["groups"]}
        self.assertEqual([], groups["主攻"])
        self.assertEqual(["中证消费", "中证红利"], groups["试探"])
        ...
        self.assertEqual("2026-07-15", innovation_drug["previousDataDate"])
        self.assertEqual(-1, innovation_drug["rankMovement"])
        self.assertEqual(1.01, innovation_drug["volumeRatio"])
        ...
        self.assertEqual(-2, directions["半导体"]["rankMovement"])
    ...
    def test_direction_detail_exposes_evidence_and_lifecycle(self):
        ...
        self.assertTrue(innovation_drug["sourceImage"].endswith("sector_5784b93f.jpg"))
    ...
    def test_snapshot_includes_display_context_without_article_body(self):
        ...
        self.assertIn("A股宽基全部跌破20日线", snapshot["marketSummary"])
        directions = {row["name"]: row for row in snapshot["directions"]}
